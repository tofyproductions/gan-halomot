 ...
 * ולא עוזר: זה לא ייצוא נרשמים שבור, זה דוח אחר לגמרי, וההמשך הנכון הוא לחזור
 * לפורטל ולהוריד את ייצוא הנרשמים — לא לחפש עמודה חסרה.
 *
 * WHAT MUST HOLD:
 *   - כותרות ייצוא החוזים  → WRONG_EXPORT_TYPE, עם הסבר איזה דוח כן צריך;
...
/** contracts_export_*.xlsx — one row per contract, no parent anywhere in it. */
...
function main() {
  ...
  head('בדיקה 1 — ייצוא החוזים');
  {
    ...
    ok(err?.error === WRONG_EXPORT_MESSAGE, '1d ההודעה מסבירה איזה דוח כן צריך', err?.error);
