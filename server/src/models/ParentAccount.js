...
const parentAccountSchema = new mongoose.Schema({
   ...
   */
  /**
   * התראה על תמונות חדשות — פעם ביום, ולא יותר.
   *
   * 100 תמונות ביום מארבעה סניפים זו התראה כל כמה דקות. תוך שבוע ההורים
   * יכבו התראות, וכשיכבו — יכבו גם את אלה על תשלומים ואיסוף. לכן הודעה אחת
   * ביום, על הילד שלהם, עם מספר.
   *
   * `last_sent_at` הוא מה שמגדיר את חלון הספירה, ולא "היום": גננת שמעלה
   * ב-21:00 את תמונות הבוקר הייתה נספרת ליום שההודעה שלו כבר יצאה, והתמונות
   * האלה לא היו מוזכרות לעולם.
   */
  photo_digest: {
    mode: { type: String, enum: ['daily', 'weekly', 'off'], default: 'daily' },
