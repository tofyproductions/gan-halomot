...
const punchSchema = new mongoose.Schema({
  ...
  // 'closure_completion' = generated by services/closureCompletion.js for a
  // committed day inside a branch's Holiday closure window (e.g. the gan
  // shutting early at the end of the school year) where she has no real punch.
  // Counted normally for hourly employees (it drives their pay like any other
  // punch); for global (תקן) employees payrollCalc.js excludes it from hours —
  // she is paid via a separate "בונוס" line instead (see payrollMonth.controller.js),
  // so the row only serves as a visible marker in the attendance grid.
  timestamp_source: {
    type: String,
    enum: ['device', 'agent_received_at', 'manual', 'fixed_schedule', 'closure_completion'],
