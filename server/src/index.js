...
connectDB().then(() => {
  const server = app.listen(env.PORT, () => {
    ...
    // זיהוי פנים בתמונות. התור מזדיין כאן אבל **כבוי** עד שמדליקים את
    // ההגדרה `face_recognition_enabled` — כך אפשר להריץ שבוע לימוד בסניף
    // אחד, ולכבות מיד אם משהו מתנהג רע, בלי דיפלוי.
    //
    // הוא שומר על עצמו רזה: כל עוד ההגדרה כבויה זו קריאת Setting אחת כל
    // 20 שניות ויציאה. המודלים (191 מגה) נטענים רק כשבאמת מגיעה תמונה
    // ראשונה לסריקה, ולא בעלייה של השרת.
