...
export default function Header() {
  ...
  // The door to the new interface, from inside the old one. It has to exist
  // here too: somebody who answered "לא עכשיו" is sitting in THIS header, and
  // was told the switch would be waiting in settings.
    ...
    <AppBar position="sticky" sx={{
        ...
        <Stack direction="row" alignItems="center" spacing={0.3} sx={{ display: { xs: 'none', md: 'flex' } }}>
          ...
          {user && (
            <Stack direction="row" alignItems="center" spacing={0.5} sx={{ ml: 0.5 }}>
              ...
              <Tooltip title={isNew ? 'חזרה לעיצוב הישן' : 'מעבר לעיצוב החדש'}>
                <IconButton size="small" onClick={() => switchTo(isNew ? 'classic' : 'new')} sx={{ color: '#B4540A' }}>
                  <ViewQuiltIcon sx={{ fontSize: '1rem' }} />
                </IconButton>
              </Tooltip>
      ...
      >
        ...
        <List>
          ...
          <ListItem disablePadding>
            <ListItemButton
              onClick={() => { setDrawerOpen(false); switchTo(isNew ? 'classic' : 'new'); }}
              sx={{ minHeight: 48 }}
            >
              <ListItemIcon sx={{ minWidth: 40, color: '#B4540A' }}><ViewQuiltIcon /></ListItemIcon>
              <ListItemText primary={isNew ? 'חזרה לעיצוב הישן' : 'מעבר לעיצוב החדש'} />
            </ListItemButton>
          </ListItem>
          <ListItem disablePadding>
